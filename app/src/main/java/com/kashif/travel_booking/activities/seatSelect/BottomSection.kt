 ...
 */
@Composable
fun BottomSection(seatCount:Int,selectedSeats:String,totalPrice:Double,onConfirmClick:()->Unit,modifier: Modifier){

    Column(modifier=modifier.fillMaxWidth().height(180.dp).background(colorResource(R.color.darkPurple)).padding(vertical = 16.dp)) {
        ...
        Row(modifier = Modifier.fillMaxWidth().padding(horizontal = 32.dp), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
            Column() {
                Text(text = "$seatCount Seat Selected",
                    color = Color.White,
                    fontSize = 14.sp,
                    fontWeight = FontWeight.Bold
                )
                Text(text = if(selectedSeats.isBlank())"-" else selectedSeats,
                    color = Color.White,
                    fontSize = 14.sp,
                    fontWeight = FontWeight.Bold
                )
            ...
            Text(text = "$${String.format("%.0f",totalPrice)}",
        ...
        Spacer(modifier=Modifier.height(8.dp))
        GradientButton(onCLick = onConfirmClick,"Confirm Seat")
    }
}